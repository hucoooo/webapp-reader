...
export default {
  ...
  book: {
    ...
    //搜索热词
    searchHotWord: API_HOST + '/book/search-hotwords',
    //搜索热词
    hotWord: API_HOST + '/book/hot-word',
    // 书籍搜索 (分类，书名，作者名)
    bookSearch: function (start, limit, content) { //@param content 搜索内容
      return `${API_HOST}/book/fuzzy-search?start=${start}&limit=${limit}&v=1&query=${content}`
    },
    bookSearchRealTime: function (query) {
      return `${API_HOST}/book/auto-complete?query=${query}`
    }
  },
  rank: {
    // 排名分类
  ...
  comment: {
    ...
    //长评
    bookReviews: function (book_id, page_num, start_num) {  // @param book_id 书籍id
      return `${API_HOST}/post/review/by-book?book=${book_id}&limit=${page_num}&start=${start_num}`
    },
    //用户评论
    userReview: function (user_id, start, limit) {
      return `${API_HOST}/post/review/${user_id}/comment/?start=${start}&limit=${limit}`
    }
  },
  bookList: {
